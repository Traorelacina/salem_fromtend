          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '1.25rem', alignItems: 'start' }}>
            ...
            <Card>
              <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <h3 style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text)', fontFamily: "'Lexend', sans-serif" }}>Messages récents</h3>
                  <p style={{ color: 'var(--text-3)', fontSize: '0.78rem', marginTop: '2px' }}>Dernières demandes de contact</p>
                </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {contacts.length === 0 ? (
                    <tr><td colSpan={4} style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-3)', fontSize: '0.85rem' }}>Aucun message</td></tr>
                  ) : contacts.map(c => (
                    <TableRow key={c.id} onClick={() => navigate('/admin/contacts')} highlight={c.status === 'new'}>
                      <Td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          <div>
                            <div style={{ fontWeight: c.status === 'new' ? 700 : 500, fontSize: '0.84rem', color: 'var(--text)' }}>{c.name}</div>
                            <div style={{ fontSize: '0.72rem', color: 'var(--text-3)' }}>{c.email}</div>
                          </div>
                        </div>
                      </Td>
                      <Td style={{ color: 'var(--text-2)', maxWidth: '200px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <Card style={{ padding: '1.25rem' }}>
                <h3 style={{ fontWeight: 700, fontSize: '0.92rem', color: 'var(--text)', fontFamily: "'Lexend', sans-serif", marginBottom: '1rem' }}>Actions rapides</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {quickActions.map(({ label, to, Icon, color }) => (
                    <button key={to} onClick={() => navigate(to)}
                      style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '0.7rem 0.85rem', background: '#f8fafc', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', cursor: 'pointer', transition: 'all 0.15s', textAlign: 'left', width: '100%' }}
                      ...
                      <div style={{ width: '30px', height: '30px', borderRadius: '7px', background: 'var(--primary-lt)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Icon size={14} style={{ color: 'var(--primary)' }} />
                      </div>
                      <span style={{ fontSize: '0.82rem', fontWeight: 500, color: 'var(--text)', flex: 1 }}>{label}</span>
              ...
              {/* Messages summary */}
              <Card style={{ padding: '1.25rem' }}>
                <h3 style={{ fontWeight: 700, fontSize: '0.92rem', color: 'var(--text)', fontFamily: "'Lexend', sans-serif", marginBottom: '1rem' }}>Statuts messages</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                  {[
                    { label: 'Nouveaux',  value: stats?.contacts?.new,      color: '#f59e0b', bg: '#fef3c7' },
                    { label: 'Lus',       value: stats?.contacts?.read,     color: '#2563eb', bg: '#dbeafe' },
                    { label: 'Répondus',  value: stats?.contacts?.replied,  color: '#16a34a', bg: '#dcfce7' },
                    { label: 'Archivés',  value: stats?.contacts?.archived, color: '#64748b', bg: '#f1f5f9' },
